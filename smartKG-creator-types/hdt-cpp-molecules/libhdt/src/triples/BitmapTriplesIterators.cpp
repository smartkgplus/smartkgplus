...
size_t BitmapTriplesSearchIterator::estimatedNumSubjects()
{
	 if(patX!=0){
		 if (maxZ>0) //found
				 return 1;
		 ...
		 return adjY.countListsX();
	 }

}

 size_t numDifferentElements(AdjacencyList* adj, size_t min,size_t max){
	map<unsigned int,int> seen;
...
size_t BitmapTriplesSearchIterator::estimatedNumPredicates(){
	if(patX!=0) {
		return (maxY-minY);
	}
	else{
		//iterate and remove duplicates
		return numDifferentElements(&adjY,minY,maxY);
	}
	return 0;
}
size_t BitmapTriplesSearchIterator::estimatedNumObjects(){
	if(patX!=0 && patY!=0) {
		return (maxZ-minZ);
	}
	else{
		//iterate and remove duplicates
		return numDifferentElements(&adjZ,minZ,maxZ);
	}
	return 0;
}

vector<unsigned int> BitmapTriplesSearchIterator::getPredicates(){
	vector<unsigned int> ret;
	for (int i=posY;i<nextY;i++){
		ret.push_back(adjY.get(i));
	}
	return ret;
}
...
size_t MiddleWaveletIterator::numDifferentObjs(){
	map<unsigned int,int> seen;
	for (int oc=predicateOcurrence;oc<=numOcurrences;oc++){
		size_t pos = predicateIndex->getAppearance(patY, oc); // pos is the position of the predicate in the Y array
		for (size_t posO=adjZ.find(pos);posO<=adjZ.last(pos);posO++){ // iterate all positions of the object
			seen[adjZ.get(posO)]=1; //get the object associated to the each  position
		}
...
size_t MiddleWaveletIterator::estimatedNumSubjects(){
	if (patY!=0){
...
size_t MiddleWaveletIterator::estimatedNumPredicates(){
	if (patY!=0){
		return 1; //shortcut, we assume it is found, otherwise there are exceptions before
	}
	return 0;
}
size_t MiddleWaveletIterator::estimatedNumObjects(){
	if (patY!=0){
			return numDifferentObjs();
	}
	return 0;
}
...
size_t ObjectIndexIterator::numDifferentPreds(size_t min,size_t max){
	map<unsigned int,int> seen;
	for (int i=min;i<=max;i++){
		seen[getY(i)]=1; //get the predicate associated to the index position
	}
	return seen.size();
}

size_t ObjectIndexIterator::numDifferentSubjs(size_t min,size_t max){
	map<unsigned int,int> seen;
	for (int i=min;i<=max;i++){
		seen[adjY.findListIndex(adjIndex.get(i))+1]=1; //get the subject associated to predicate list in the index position
	}
	return seen.size();
}
...
size_t ObjectIndexIterator::estimatedNumSubjects(){
	if ((maxIndex >= 0) && (maxIndex>=minIndex)){ //found
		if (patZ!=0 && patY!=0) { //shortcut
			return maxIndex-minIndex+1; //if predicate and object are given, the number of subjects is the number of positions
		}
		else{
			return numDifferentSubjs(minIndex,maxIndex);
		}
	}
	return 0;
}

size_t ObjectIndexIterator::estimatedNumPredicates(){
	if ((maxIndex >= 0) && (maxIndex>=minIndex)){ //found

		if (patZ!=0 && patY!=0) { //shortcut
			return 1;
		}
		else{ //iterate and remove duplicates
			return numDifferentPreds(minIndex,maxIndex);
		}
	}
	return 0;
}
size_t ObjectIndexIterator::estimatedNumObjects(){
	if ((maxIndex >= 0) && (maxIndex>=minIndex)) //found
		return 1;
