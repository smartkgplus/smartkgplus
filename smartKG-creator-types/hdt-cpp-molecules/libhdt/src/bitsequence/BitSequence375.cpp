...
size_t BitSequence375::load(const unsigned char *ptr, const unsigned char *maxPtr, ProgressListener *listener){
    ...
    //array = (char *) &ptr[count];
    if (sizeBytes>=4){
    	array = (size_t *) &ptr[count];
    }
    else{ // fix problem with padding bits
    	size_t* newData = (size_t *)&ptr[count];
    	if (sizeBytes==3)
    		array[0]=(size_t)(newData[0]&(unsigned int)0x00FFFFFF);
    	else if (sizeBytes==2)
    		array[0]=(size_t)(newData[0]&(unsigned int)0x0000FFFF);
    	else if (sizeBytes==1)
    		array[0]=(size_t)(newData[0]&(unsigned int)0x000000FF);
    }
	isMapped = true;
	count += sizeBytes;
