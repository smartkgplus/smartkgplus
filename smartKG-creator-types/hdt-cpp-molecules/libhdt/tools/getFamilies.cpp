...
std::unordered_set<unsigned int> filterInClasses; //classes that should be filtered in
unsigned int idType = 0;
map<unsigned int, vector<unsigned int>> classesPerSubject; //for each subject with allowed classed, the allowed classes included in it
	...
	*out << "  \"families\": [ " << endl;
	for (int i = start; i < end; i++) // iterate families
		...
		*out << "      \"name\": \"" << splitFileString << (i + 1) << ".hdt\""
		...
		// export classes of the family if required
		if (filterClasses) {
			if (subjectsPerFamilies.find(i+1)!=subjectsPerFamilies.end()){
				vector<unsigned int> subjects = subjectsPerFamilies.find(i+1)->second;
				//get classss per subject
				std::unordered_set<unsigned int> classesUnique;
				for (int ss=0; ss<subjects.size();ss++) {
					if (classesPerSubject.find(subjects[ss])!= classesPerSubject.end()) {
						vector<unsigned int> classes = classesPerSubject.find(subjects[ss])->second;
						for (int cl = 0; cl < classes.size();cl++) {
							classesUnique.insert(classes[cl]);
						}
					}
				}
				//print
				if (classesUnique.size()>0) {
					*out << "      \"classesSet\": [ ";	
					int sizeClasses =1;
					for (auto itr = classesUnique.begin(); itr != classesUnique.end(); ++itr) {
						unsigned int classPrint= *itr;
						*out << "\"" <<hdt_file1->getDictionary()->idToString(classPrint, OBJECT) << "\"";
						if (sizeClasses < classesUnique.size())
							*out << ", ";
						sizeClasses++;
					}
					*out << " ]," << endl;
...
int main(int argc, char **argv)
{
	...
	string classFile; // file to read the classes to filter in 
	...
	while ((c = getopt(argc, argv, "hve:o:s:iuGdm:ScP:L:H:C:")) != -1)
	{
		switch (c)
		{
		case 'h':
	...
	try
	{
		...
		if (filterClasses) { // read the classes to filter 
			// Open the File 
			std::ifstream in(classFile);                                                                                          
			string line;                                                                                  
			if (!in) {                                                                      
				cerr << "Cannot open the File : "<<classFile<<endl;                                   
				exit(-1);                                                                             
			}                                                                                             
			...
			{        
				if(line.size() > 0) { 
					// search ID
					//cout<<line<<endl;
					//cout<<hdt_file1->getDictionary()->stringToId(line,OBJECT)<<endl;
					filterInClasses.insert(hdt_file1->getDictionary()->stringToId(line,OBJECT));

				}
			}
			cout<<"These classes will be filtered in:"<<endl;
			for (auto itr = filterInClasses.begin(); itr != filterInClasses.end(); ++itr) {
				cout<< *itr <<endl; 
				cout << hdt_file1->getDictionary()->idToString(*itr, OBJECT)<<endl<<endl;
			}
			// look for RDF type
			idType = hdt_file1->getDictionary()->stringToId("http://www.w3.org/1999/02/22-rdf-syntax-ns#type", PREDICATE);
			cout << "The type predicate has the id:" << idType << endl;
			cout << hdt_file1->getDictionary()->idToString(idType, PREDICATE)<<endl;
		}
		...
		if (includeInfrequentPredicates==false){
			for (size_t i=1;i<=hdt_file1->getDictionary()->getNpredicates();i++){
				...
				if (cutMassivePredicates && (numOccs_predicate>=MAX_CUT_PREDICATES) && (!filterClasses || i!=idType )){ //Do include type if we filter Classes
					num_cutPreds++;
		...
		while (it1->hasNext())
		{
			...
			{
				if ((predsofFamily.size()>0)&& (!filterClasses||isAllowedClass)){ // condition to assure that we have something, as maybe all infrequent predicate have been filtered out
			...
			if (!predbanned){
				...
				if (filterClasses && triple->getPredicate() == idType) { //filter classes if activated

					if (filterInClasses.find(triple->getObject())!=filterInClasses.end()) {
						//cout << "filter"<<endl;
						//cout << hdt_file1->getDictionary()->idToString(triple->getSubject(), SUBJECT)<<endl;
						isAllowedClass = true;
						classesPerSubject[triple->getSubject()].push_back(triple->getObject());
					}
				}
			}
		...
		if (filterClasses){ 
			cout<<"Number of classes filtered:"<<filterInClasses.size()<<endl;
			cout<<"Number of subjects filtered in classes:"<<classesPerSubject.size()<<endl;
		}
		/*
		...
		if (groupFilterSubjects){ // if we restrict the families with a min number of subjects, then first identify these families
			...
			int numPredicatesCore=0;
			for (int i=0;i<predicatesPerFamily.size();i++){
				if (subjectsPerFamilies[i+1].size()>=MIN_SUBJECT_GROUP){ //11399 //0.01% of number of subjects
						familiesLargerSubjects.insert(i+1);
						numPredicatesCore+=predicatesPerFamily[i+1].size();
				}
			  //  cout<<(i+1)<<";"<<numTriplesperFamily[i+1]<<";"<<subjectsPerFamilies[i+1].size()<<";"<<predicatesPerFamily[i].size()<<endl;
			}
			cout<<familiesLargerSubjects.size()<< " core families present in >= "<<MIN_SUBJECT_GROUP<<" ("<<PERCENTAGE_SUBJECT_GROUPS<<" percent of the subjects)"<<endl;
			cout<<numPredicatesCore<< " predicates in the core families"<<endl<<endl;
	...
	//				cout<<"current familyToMap[i+1] size:"<<familyToMap[i+1].size()<<endl;
			...
			while (itSecond->hasNext())
			{
				...
				if (triple->getSubject() != prevSubject){
				   ...
				   unsigned int famID=0;
					// filter infrequent predicates
				   if (!filterClasses || (classesPerSubject.find(triple->getSubject())!=classesPerSubject.end())) { //filter by class if needed
					if (!includeInfrequentPredicates){
						string fam = getFamilyStr(&predicates);
						itString = mapFilterInfrequent.find(fam);
						if(itString != mapFilterInfrequent.end()){ //exists in cache, get the filter
				...
				if (!jumpSubject){ // it is a merged family and, if requred, it belongs to an allowed class 
