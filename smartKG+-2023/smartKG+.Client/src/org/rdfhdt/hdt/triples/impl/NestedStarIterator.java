...
public class NestedStarIterator {
    ...
    private void bufferNext() {
        while(true) {
            ...
            if (!vars.first.equals(""))
                bindings.put(vars.first, (int) tid.getPredicate());
            if (!vars.second.equals(""))
                bindings.put(vars.second, (int) tid.getObject());
    ...
    private void setNextSelected() {
        ...
        if (selected.getPredicate() == 0 && currentBindings.containsKey(vars.first))
            selected.setPredicate(currentBindings.get(vars.first));
        if (selected.getObject() == 0 && currentBindings.containsKey(vars.second))
            selected.setObject(currentBindings.get(vars.second));
    }
    ...
    private TripleID replace(TripleID triple, Tuple<String, String> vars) {
        ...
        if (triple.getPredicate() == 0 && currentBindings.containsKey(vars.first))
            triple.setPredicate(currentBindings.get(vars.first));
        if (triple.getObject() == 0 && currentBindings.containsKey(vars.second))
            triple.setObject(currentBindings.get(vars.second));
        return triple;
    }
}
