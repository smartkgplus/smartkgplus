...
std::unordered_set<string> filterInClasses;
void help()
{
	...
	cout
		...
		<< endl;
	cout
		<< "\t-C <classFile>\t\t\t Filter in only the provided classes (one per line)."
		<< endl;
	cout
...
int main(int argc, char **argv)
{
	...
	while ((c = getopt(argc, argv, "hve:o:s:iuGdm:ScP:L:H:qC:")) != -1)
	{
		switch (c)
		{
		case 'h':
...
//		}
		...
		if (!groupFilterSubjects){
			...
			cout<<"  - With Subject cut, these families are present in >= "<<MIN_SUBJECT_GROUP<<" ("<<PERCENTAGE_SUBJECT_GROUPS<<" percent of the subjects)"<<endl<<endl;
			 ...
			 std::unordered_set<unsigned int> predicatesUnique;
                                for (int pp=0; pp<predicatesPerFamily.size();pp++) {
                                        vector<unsigned int> predicates= predicatesPerFamily[pp];
                                        for (int p = 0; p < predicates.size();p++) {
                                                if (predicatesUnique.find(p)== predicatesUnique.end()) {
                                                        predicatesUnique.insert(p);
                                                }
                                        }
                                }
                                cerr<< " Initial number of predicates: "<<predicatesUnique.size()<<endl;

			// now iterate only the core families 
			std::unordered_set<unsigned int> predicatesUniqueLarge;
			for (auto itr = familiesLargerSubjects.begin(); itr != familiesLargerSubjects.end(); ++itr) {
                        	unsigned int targetFamily= *itr;
                                        vector<unsigned int> predicates= predicatesPerFamily[targetFamily];
                                        for (int p = 0; p < predicates.size();p++) {
                                                if (predicatesUniqueLarge.find(p)== predicatesUniqueLarge.end()) {
                                                        predicatesUniqueLarge.insert(p);
                                                }
                                        }
                                }
                                cerr<< " Core number of predicates: "<<predicatesUniqueLarge.size()<<endl;
