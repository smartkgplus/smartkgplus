...
//      // FIXME: Allow a filter here.
        ...
        Function<TripleID, BindingHDTId> binder = new Function<TripleID, BindingHDTId>()
        {
            ...
            public BindingHDTId apply(TripleID triple)
            {
                ...
                if (var[0] != null && !insert(var[0], new HDTId((int)triple.getSubject(), TripleComponentRole.SUBJECT, dictionary), output)) {
                    return null;
                }
                if (var[1] != null && !insert(var[1], new HDTId((int)triple.getPredicate(), TripleComponentRole.PREDICATE, dictionary), output)) {
                    return null;
                }
                if (var[2] != null && !insert(var[2], new HDTId((int)triple.getObject(), TripleComponentRole.OBJECT, dictionary), output)) {
                    return null;
                }
