...
unsigned int maxCombination = 3;
unsigned int topCombinations = 1000;
bool includetopCombinations = false;
...
void help()
{
	...
	cout
			...
			<< endl;
	cout
				<< "\t-T <X> \t\t\tinclude Top X combinations (e.g. 1000) of 3 elements"
				<< endl;
	cout
...
int main(int argc, char **argv)
{
	...
	while ((c = getopt(argc, argv, "hve:o:s:iuGdm:ScT:")) != -1)
	{
		switch (c)
		{
		case 'h':
		...
		case 'c':
			...
			break;
		case 'T':
			topCombinations = atoi(optarg);
			includetopCombinations = true;
			break;
		default:
	...
	try
	{
		...
		if (includeInfrequentPredicates==false){
			...
			if (cutMassivePredicates){
				...
				cout<<"Remaining predicates:"<<hdt_file1->getDictionary()->getNpredicates()-num_infrequentPreds-num_cutPreds<<endl;
			}
			...
			if (groupFilterSubjects){ // if we group restricting to a minimum number of covered subjects
				...
				for (int j=0;j<predicatesPerFamily.size();j++){
					...
					if (found)
						numFound++;
					/*
					 * We do not materialize the families without intersection, otherwise the results are not complete
					 *
					 * else{
	...
	//			for (int i=0;i<familiesLargerSubjects_fromMerge.size();i++)
		//			familiesLargerSubjects_total.push_back(familiesLargerSubjects_fromMerge[i]);
				...
				if (includetopCombinations){
					cout<< "Let's compute the top "<<topCombinations<<" combinations of "<<maxCombination<<" predicates out of the large families";
					// first, compute how many predicates are out;


					unsigned int count=0;
					for (size_t i=0;i<hdt_file1->getDictionary()->getNpredicates();i++){
						bool found=false;
					    unsigned int pred = i+1;
					    // if it is one of the desired predicate, neither infrequent nor massive
					    if ((infrequent_predicates.find(pred)==infrequent_predicates.end())&& (cut_predicates.find(pred)==cut_predicates.end())){
							for (int i = 0; i < familiesLargerSubjects_total.size(); i++){ // iterate families
								vector<unsigned int> predTest = predicatesPerFamily[familiesLargerSubjects_total[i]-1];
								for (int kk=0;kk<predTest.size();kk++){
									if (predTest[kk]==pred){
										found = true;
									}
								}
							...
							if (!found){
								cout<<"Desired predicate not found in large families:"<<hdt_file1->getDictionary()->idToString(pred,PREDICATE)<<endl;
								count++;
							}
					    }
					}
					cout<<"Missing "<<count<< "desired predicates in large families"<<endl;
